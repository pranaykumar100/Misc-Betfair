...
namespace BFCompressedParser.Parsers
{
    internal static class RunnerPricesParser
    {
        internal static IEnumerable<RunnerPrices> Parse(IEnumerable<string> runners)
        {
            ...
            {
                string[] fields = runner.Split('|');

                // Info is in fields[0]
                string[] info = fields[0].Split('~');

                IEnumerable<Price> allPrices = fields.Length == 3
        ...
        private static IEnumerable<Price> ParseStandardPrices(string compressedLayPrices, string compressedBackPrices)
        {
            ...
            string trimmedLayPrices = compressedLayPrices.RemoveSingleTrailingChar('~');
            string trimmedBackPrices = compressedBackPrices.RemoveSingleTrailingChar('~');

            if (string.IsNullOrEmpty(trimmedLayPrices) && string.IsNullOrEmpty(trimmedBackPrices))
                yield break;

            string[] prices = (trimmedLayPrices + "~" + trimmedBackPrices).Split('~');

            for (int i = 0; i < prices.Length; i += 4 /* 4 elements per runner */)
            {
                double price = double.Parse(prices[i]);
                double available = double.Parse(prices[i + 1]);
                var betType = (BetTypeEnum) Enum.Parse(typeof (BetTypeEnum), prices[i + 2]);
                int depth = int.Parse(prices[i + 3]);
        ...
        private static IEnumerable<Price> ParseCompletePrices(string compressedPrices)
        {
            ...
            string trimmed = compressedPrices.RemoveSingleTrailingChar('~');

            if (string.IsNullOrEmpty(trimmed))
                yield break;

            string[] prices = trimmed.Split('~');
            Debug.Assert(prices.Length%5 == 0);

            for (int i = 0; i < prices.Length; i += 5 /* 5 elements per runner */)
            {
                double price = double.Parse(prices[i]);

                double availToBack = double.Parse(prices[i + 1]);
                if (availToBack > 0.0)
                    yield return new Price {amountAvailable = availToBack, betType = BetTypeEnum.B, price = price};

                double availToLay = double.Parse(prices[i + 2]);
                if (availToLay > 0.0)
